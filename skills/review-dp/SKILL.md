...
argument-hint: "A file path or class to review for deep-module design principles"
---

# Deep Module Design Review

Analyze a C# source file (one or more classes/interfaces) against deep-module design principles. Produce a structured report identifying design strengths, violations, and actionable improvements.
...
### Deep Module
Provides powerful functionality behind a simple interface. Depth = functionality ÷ interface complexity. The best modules maximize hidden functionality while minimizing interface surface.

### Shallow Module
Interface complexity is comparable to implementation complexity — it hides almost nothing. The cost of learning the interface negates the benefit of hiding internals.
...
Read the target file. For each class/type found, record:
...
|---|---|
| **Standalone module** | Has its own meaningful abstraction; consumers use it through its interface without knowing internals |
| **Subdivision / helper** | Exists only to serve another class; has no independent consumers; would not make sense on its own |

If subdivision classes outnumber standalone modules, flag potential **classitis**.
...
|-------|------|----------------|-------|---------|
| ...   | Module / Subdivision | count | Deep / Shallow | ✅ / ⚠️ / ❌ |
